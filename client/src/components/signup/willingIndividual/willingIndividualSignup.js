...
const WillingIndividualSignup = () => {
  ...
  async function submitHandler(event) {
    ...
    if (!name || !password || !email || !contact) {
      // only basic check since signup backend accepting empty value too
    ...
    await SignUpInd(name, password, email, contact)
      .then((res) => {
        // since signup backend doesn't respond with other than status code, we take response OK as fine
    ...
    console.debug(
      "SignUp backend currently doesn't accept these: ",
      lastName,
      yourhelp,
      address,
      willhelp,
