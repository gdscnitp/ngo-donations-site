...
db.once('open', () => {
	...
	requestWatcher.on('change', (delta) => {
		...
		if (opType === 'insert') {
			...
			io.emit('feed:update', { id: delta.documentKey._id, updateDescription: delta.updateDescription });
			...
			console.warn(`[${Date.now()}] ⚠⚠ Collection name has been changed from ${delta.to.db}.${delta.to.coll} ⚠⚠`);

		} else if (opType === 'invalidate') {	// caused by drop or rename
			...
			console.error(`[${Date.now()}] ‼‼🔺 Watch on collection ${reqModel.collection.name} INVALIDATED due to ${opType}. Resume Token is ${delta._id}`);
		}
	});
 ...
 *  @example_url -> `https://<website>/feeds/get?limit=25`
 *  @params -> Accepts a `limit` parameter (for how many feeds to return), if not given, defaults to 100
 *  @request_body -> {}
 *
...
app.get('/get', (req, res) => {
	...
	const num = Number(req.query.limit) || max_limit;
...
app.get('/get/:id', (req, res) => { // get more details of a particular feed item
